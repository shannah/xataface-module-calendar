<?php
class actions_calendar_event_moved {
	function handle($params){
		...
		try {
			...
			if ( PEAR::isError($startAtt) ) throw new Exception('Failed to find start time field in this event.', 500);
			
			if ( !$record->checkPermission('edit', array('field'=>$startAtt)) ){
				throw new Exception('Failed to move record because you don\'t have permission to change the start time field.', 400);
			...
			if ( @$_POST['-calendar-day-delta'] ){
				...
				$currStart = $record->strval($startAtt);
				$currEnd = $record->strval($endAtt);
				$newStart = date('Y-m-d H:i:s', strtotime($deltaSign.$dateDelta.' day', strtotime($currStart)));
			...
			if ( @$_POST['-calendar-minute-delta'] ){
				...
				$newStart = date('Y-m-d H:i:s', strtotime($minuteDelta.' minutes', strtotime($currStart)));
			...
			if (@$_POST['-calendar-all-day'] == 'true' ){
				// This is set to all day.
				$alldayAtt = $ontology->getFieldname('allday');
				
				if ( !$alldayAtt or PEAR::isError($alldayAtt) ){
					throw new Exception('Could not find allday attribute in the field.  Please mark a field with event.allday in the fields.ini file.', 500);
